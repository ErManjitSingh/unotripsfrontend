...
async function main() {
  const tarPath = join(root, "meta-kerala-deploy.tar.gz");
  execSync(`tar -czf "${tarPath}" -C meta kerala`, { stdio: "inherit" });
  const conn = await connect();
  console.log("Uploading Kerala meta...");
  await upload(conn, tarPath, "/root/meta-kerala-deploy.tar.gz");
  ...
  await exec(conn, [
    ...
    "rm -rf /var/www/unotrips-meta/kerala",
    "tar -xzf /root/meta-kerala-deploy.tar.gz -C /var/www/unotrips-meta",
    "chown -R www-data:www-data /var/www/unotrips-meta/kerala || true",
    "ln -sf /etc/nginx/sites-available/unotrips.com /etc/nginx/sites-enabled/unotrips.com",
    "nginx -t",
    "systemctl reload nginx",
    "curl -s -o /dev/null -w 'kerala_page:%{http_code}\\n' https://unotrips.com/meta/kerala/",
    "curl -s https://unotrips.com/meta/kerala/ | head -c 500",
  ].join(" && "));
  conn.end();
  console.log("\nKerala meta deploy done.");
}
