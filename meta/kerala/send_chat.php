...
$destination = trim((string) ($input['destination'] ?? $input['destinationy'] ?? 'Kerala'));
...
try {
  ...
  if ($userPhone !== '') {
    uno_crm_push_lead([
      'name' => $userName !== '' ? $userName : 'Kerala Chatbot Lead',
      'phone' => $userPhone,
      'email' => $userEmail,
      'destination' => $destination !== '' ? $destination : 'Kerala',
      'source' => 'Kerala Chatbot',
      'sourceLabel' => 'Kerala Chatbot',
      'chat' => $lines,
      'captureType' => 'chatbot',
      'channel' => 'meta',
    ]);
