...
export function LehTourLanding({ h1 }: Props) {
  ...
  return <main className="leh-root" style={{ "--leh-progress": `${progress}%` } as CSSProperties}>
    ...
    <section className="leh-section leh-section-trust"><div className="leh-trust-scroll" aria-label="Trust highlights"><div className="leh-trust-cards">{LEH_TRUST_CARDS.map((item) => <div className="leh-card leh-trust-card" key={item.label}><i>{item.icon === "star" ? "★" : item.icon === "shield" ? "◈" : "✦"}</i><strong>{item.value}</strong><span>{item.label}</span></div>)}</div></div></section>
