...
export function SummerEscapesSection({
  ...
  return (
    <section
      className={cn(
        "border-b border-slate-200/80 bg-slate-50 py-5 sm:bg-slate-100 sm:py-8 lg:py-10",
        className,
      )}
    >
      <div className="mx-auto w-full max-w-[1320px] px-4 sm:px-4 lg:px-6">
        <div className="rounded-[22px] bg-white p-4 shadow-[0_18px_42px_-28px_rgba(15,23,42,0.45)] sm:rounded-3xl sm:p-6 lg:p-8">

          {/* ── Header ── */}
          <div className="mb-4 flex flex-col gap-3 sm:mb-6 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0">
              <h2 className="flex items-center gap-2 text-[21px] font-black leading-tight tracking-tight text-slate-900 sm:text-2xl lg:text-[1.75rem]">
                <span className="text-2xl leading-none">🌴</span>
                Summer Escapes
                <span className="text-lg text-primary leading-none">✦</span>
              </h2>
              <p className="mt-1 text-[15px] text-slate-500 sm:text-[0.9375rem]">
                Handpicked destinations for your next getaway.
              </p>
              <div className="mt-2 flex items-center gap-1.5 text-[13px] font-semibold text-primary sm:text-sm">
                <Tag className="h-3.5 w-3.5 shrink-0" strokeWidth={2.5} />
                Up to 40% OFF on unforgettable experiences
              </div>
            </div>

            <div className="hidden shrink-0 items-center gap-3 self-end sm:flex sm:self-auto">
              <Link
                href="/packages"
              ...
              return (
                <Link
                  ...
                  className={cn(
                    ...
                    "w-[42%] min-w-[138px] sm:w-[31%] sm:min-w-[200px] lg:w-[calc(20%-10px)] lg:min-w-[200px]",
                ...
                >
                  ...
                  <div className="absolute left-2.5 top-2.5 z-10 sm:left-3 sm:top-3">
                    <span className="flex items-center gap-1 rounded-full border border-primary/25 bg-white px-2 py-0.5 text-[10px] font-bold text-primary shadow-sm sm:px-2.5 sm:py-1 sm:text-[11px]">
                  ...
                  {/* Bottom info */}
                  <div className="absolute inset-x-0 bottom-0 z-10 p-2.5 sm:p-4">
                    <div className="mb-0.5 flex items-center gap-1">
                      <MapPin className="h-3.5 w-3.5 shrink-0 text-white/70" strokeWidth={2} />
                      <p className="text-[15px] font-bold leading-tight text-white sm:text-lg">
                        {card.label}
                      </p>
                    </div>
                    <p className="mb-2 text-[10px] text-white/65 sm:text-[11px]">{countLabel}</p>

                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-baseline gap-1.5">
                        <span className="text-[13px] font-extrabold text-primary sm:text-base">
                          {card.startingPrice}
                        </span>
                        <span className="hidden text-[11px] text-white/45 line-through sm:inline">
                          {card.originalPrice}
                        </span>
                      </div>
                      <span className="shrink-0 rounded-full bg-white px-2.5 py-1 text-[10px] font-semibold text-slate-800 shadow-sm sm:px-3 sm:py-1.5 sm:text-xs">
                        Explore →
                      </span>
                    </div>
                  </div>
